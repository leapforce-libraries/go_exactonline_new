...
// LogisticsItem stores LogisticsItem from exactonline
//
...
type SyncLogisticsItemsCall struct {
	urlNext string
	service *Service
}

func (service *Service) NewSyncLogisticsItemsCall(timestamp *int64) *SyncLogisticsItemsCall {
	selectFields := utilities.GetTaggedTagNames("json", LogisticsItem{})
	url := service.url(fmt.Sprintf("Logistics/Items?$select=%s", selectFields))
	if timestamp != nil {
		url = fmt.Sprintf("%s&$filter=Timestamp gt %vL", url, *timestamp)
	}

	return &SyncLogisticsItemsCall{
		service: service,
		urlNext: url,
	}
}

func (call *SyncLogisticsItemsCall) Do() (*[]LogisticsItem, *errortools.Error) {
	if call.urlNext == "" {
		return nil, nil
	}
