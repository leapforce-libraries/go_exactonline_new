...
// methods
//
func NewExactOnline(clientID string, clientSecret string, bigQuery *bigquerytools.BigQuery, isLive bool) (*ExactOnline, error) {
	eo := ExactOnline{}

	http, err := http.NewHttp(clientID, clientSecret, bigQuery, isLive)
	if err != nil {
		return nil, err
	}
